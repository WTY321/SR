...
class VSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.localization_head = MuitipleRoadLoc(self.num_features, num_classes)
        #self.localization_head =make_loc_classifier(self.num_features, num_classes)
        if num_classes > 0:
            self.head = make_cls_classifier(self.num_features, num_classes)
        else:
    ...
    def forward(self, x: torch.Tensor,label=None,N=1):
        head_copy = copy.deepcopy(self.head)
        cls_copy = copy.deepcopy(self.classifier)
        layer_copy = copy.deepcopy(self.layers[-1])
        batch=x.size(0)
        ...
        x_localization=self.classifier(x_localization)
        x_saliency_all = self.localization_head(x_localization,p_label)
        #x_saliency_all = self.localization_head(x_localization)
        x_saliency = torch.zeros(batch, 1, 8, 8).cuda()
        for i in range(batch):
        ...
        x_erase = x_localization.detach() * (x_saliency)
        x_erase = x_erase.permute(0, 2, 3, 1)
        x_erase = layer_copy(x_erase)
        x_erase=cls_copy(x_erase)
        x_erase=head_copy(x_erase)
        ...
        x = self.feature_map * self.x_saliency
...
class MuitipleRoadLoc(nn.Module):
    ...
    def forward(self,x,p_label):
        ...
        return self.normalize_tensor(feat_map_a+feat_map_b+feat_map_d)
...
def load_pretrained_model(model):
    state_dict = torch.load('Model/vssmtiny_dp02_ckpt_epoch_258.pth')

    state_dict = remove_layer(state_dict, 'fc')
    strict_rule = False

    model.load_state_dict(state_dict, strict=strict_rule)
    print('complete pretrained')
    return model
def get_vmamba_model(num_classes,pretrained=True):
    model = VSSM(
        patch_size=4,in_chans=3,num_classes=num_classes,depths=[2,2,4,2],dims=96,ssm_d_state=1,ssm_ratio=2.0,ssm_rank_ratio=2.0,ssm_dt_rank="auto",ssm_act_layer="silu",ssm_conv=3,ssm_conv_bias=False,ssm_drop_rate=0.0,ssm_init="v0",forward_type="v2",
        mlp_ratio=4.0,mlp_act_layer="gelu",mlp_drop_rate=0.0,drop_path_rate=0.2,patch_norm=True,norm_layer="ln",downsample_version="v3",patchembed_version="v2",gmlp=False,use_checkpoint=False,
    )
    if pretrained:
